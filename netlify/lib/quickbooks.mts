...
export async function updateInvoiceFromBigForm(record: RegistrationRecord, fees: BigFormFeeSummary) {
  ...
  const body = {
    ...
    CustomerMemo: { value: `Your Big Form has been received. The $${record.depositCents / 100} deposit remains applied. Eligible optional competitions are billed at the Honor Roll 50% price; tickets and advertising remain full price.${pendingNote}` },
